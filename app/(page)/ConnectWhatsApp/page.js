...
function ConnectWhatsApp() {
  ...
  const loadWhatsAppNumbers = useCallback(
    async (offset = 0, showLoading = true) => {
      try {
        setIsRetrying(true);
        setLoading(true);
    ...
    <div className="font-source-sans min-h-screen flex items-start justify-center bg-[#F9FBFF] px-4 sm:px-6 lg:px-8">
      <div className="bg-white shadow rounded-lg p-6 my-5 sm:p-8 lg:p-10 w-full max-w-md sm:max-w-lg lg:max-w-2xl">
          ...
          {loading && isRetrying ? (
            ...
            accounts.map((account) => (
              ...
              >
                ...
                <div
                  className={`text-[14px] font-semibold ${getStatusColor(
                    account.status
                  )} sm:ml-auto`}
                >
                  {account.status}
        ...
        <div className="mt-6">
          <Link
            target="blank"
            href={`${process.env.NEXT_PUBLIC_STAGE_URL}/whatsapp`}
