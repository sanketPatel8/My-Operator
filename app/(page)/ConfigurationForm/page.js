...
function ConfigurationForm({ searchParams }) {
  ...
  const transformApiDataToAccounts = (apiData) => {
    ...
    return results.map((item, index) => ({
      ...
      status:
        item.phone_number_status === "connected" || item.status === "active"
          ? "Live"
          : "Inactive",
      ...
      wabaAccount: item.waba_account
        ? {
            ...
            businessVerificationStatus:
              item.waba_account.business_verification_status || "unverified",
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const matchedAccount = whatsappAccounts.find(
        (account) =>
      ...
      const payload = {
        ...
        phone_number_id:
          selectedNumber.phone_number_id || matchedAccount?.phoneNumberId || "",
        waba_id:
  ...
  const handleDelete = () => {
    const store = stripShopify(shopUrl);
    window.open(
      `https://admin.shopify.com/store/${store}/settings/apps/app_installations/app/MyOperator`,
      "_blank"
    );
    closeModal();
  };
