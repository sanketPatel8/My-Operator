...
export async function GET(req) {
  try {
    ...
    try {
      ...
      // ----- Webhook register call -----
      const registerWebhookRes = await fetch(
        `${process.env.APP_URL}/api/register-webhook`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!registerWebhookRes.ok) {
        const errorData = await registerWebhookRes.json();
        console.error(
      ...
      // Redirect user after install and webhook register
      const redirectUrl = new URL(
        "http://my-operator.vercel.app/ConfigureWhatsApp"
      );
      redirectUrl.searchParams.set("shop", shop);
