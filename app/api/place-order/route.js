...
export async function POST(req) {
  try {
    ...
    const { shop, order_id, order_status_url, payment_gateway_names, phonenumber, customer, line_items, order_number, access_token } = rows[0];
    ...
    if(status=="yes"){
      ...
      function getMappedValue(mappingField, data, customerData, line_items_data) {
      switch (mappingField) {
        case "Name":
          return (
    ...
    function buildTemplateContent(templateRows, data, customerData, line_items_data, image_id) {
    ...
    console.log("First template image:", image_id);
        ...
        const templateContent = buildTemplateContent(templateRows, data, customerData, line_items_data, image_id);
